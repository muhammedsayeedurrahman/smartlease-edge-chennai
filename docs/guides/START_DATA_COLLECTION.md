...
python tools/acoustic_data_tracker.py
```
...
# Download datasets
python tools/download_datasets.py --roboflow
```
...
2. **Download datasets:**
   ```bash
   python tools/download_datasets.py --kaggle
   ```
...
**Final validation:**
- [ ] `tools/validate_training_data.py` shows all green checkmarks
...
- Run `python tools/acoustic_data_tracker.py` to see real-time progress
...
- Run `python tools/download_datasets.py --list` to see available datasets

**After completing everything:**
- Run `python tools/validate_training_data.py` to confirm readiness
