...
| Vision segmenter (YOLOv8n-Seg via PyTorch Lite, CPU) | ✅ DONE, weak, retrained | mask mAP50 **0.2145** pooled over 686 held-out images; threshold 0.45 set from the measured specificity/recall exchange rate (39.7% clean-image specificity for 0.7pt of recall). `handoff/METRICS_MODELS.md`. |
...
| Venue AC IR pattern capture | ⛔ OPEN — brand now confirmed, protocol candidate now brand-mismatched | `WalkthroughScreen.kt`'s scripted demo path is unchanged — still the honestly-labeled generic NEC placeholder. 2026-09-12 timeline: (1) photographed a remote reading "YY-107A"; a third-party reseller listing suggested O General/Fujitsu_AC compatibility; built a real port of that protocol's 7-byte toggle frame (`FujitsuAcCandidate.kt` + `FujitsuAcCandidateTiming`, wired to `IrController.transmitFujitsuCandidate()`, exposed as "Send candidate ON/OFF" on the **Self-Test screen**, UNVERIFIED). Added a separate, clearly-labeled "Simulate result (rehearsal only)" button (sends nothing, shows a canned `[SIMULATED]` line) purely so the UI/flow can be rehearsed — **never to be shown to a judge as a real result.** (2) **A later, sharper photo of the actual venue unit shows a legible "DAIKIN" logo and a facilities asset tag "XV/DMR/4F/AC-10".** This is direct first-party evidence of the real brand, stronger than the reseller-listing inference — and it conflicts with the O General/Fujitsu lead. Daikin uses its own, different, and more complex protocol family (also documented in IRremoteESP8266, but with several sub-variants — Daikin/Daikin2/Daikin160/176/216/64 — needing a model number to pick correctly, which we don't have). **The photographed "YY-107A" remote most likely does not belong to this specific unit.** App rebuilt and installed on the connected device for on-site testing either way. **Next real step: either (a) test the existing Fujitsu candidate anyway as a data point (expected: no reaction, given the brand mismatch), or (b) find and photograph the actual Daikin remote for asset AC-10 — facilities/venue staff are the fastest path to that, not more code.** |
