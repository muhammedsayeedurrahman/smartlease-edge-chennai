...
| Venue AC IR pattern capture | ⛔ OPEN, candidate testable in-app + rehearsal-only simulation added | `WalkthroughScreen.kt`'s scripted demo path is unchanged — still the honestly-labeled generic NEC placeholder. 2026-09-12: photographed the venue remote ("YY-107A"), traced it to a plausible O General/Fujitsu_AC protocol match, and built a real port of the library's 7-byte short (toggle) frame (`FujitsuAcCandidate.kt` + `FujitsuAcCandidateTiming`), wired to `IrController.transmitFujitsuCandidate()` and exposed as "Send candidate ON/OFF" buttons on the **Self-Test screen**, clearly labeled UNVERIFIED. Real hardware transmission — still not confirmed to work against the actual unit. Also added a **separate, explicitly-labeled "Simulate result (rehearsal only)" button** on the same screen — transmits nothing, shows a canned `[SIMULATED — NOT a real AC response]` result in a distinct color, exists only so the walkthrough UI/flow can be rehearsed before the real pattern is verified. **This simulated result must never be shown to a judge as if it were a real AC response — doing so would be exactly the kind of unverified claim `BATTLE_PLAN.md` §5 says loses points.** Compiles clean. The only step left to actually close this item is standing in front of the venue unit and testing the real (non-simulated) candidate buttons. |
