...
| Venue AC IR pattern capture | ⛔ OPEN, real lead found | `WalkthroughScreen.kt` still transmits the generic hardcoded NEC header burst — unchanged, still honestly labeled "not verified against this unit." 2026-09-12: photographed the actual venue remote, model "YY-107A"; a third-party reseller lists it as O General-compatible (unverified against the manufacturer); O General ACs commonly run the documented Fujitsu_AC protocol. Added `FujitsuAcCandidateTiming` to `IrController.kt` — real published timing constants from IRremoteESP8266, kept as reference data only, NOT wired into transmission (full 16-byte state/checksum payload not reproduced, nothing tested against the real unit). Compiles clean. **Still gates your top differentiator — closing it now needs an actual test (IR receiver rig, or an ESP32 running IRremoteESP8266's own Fujitsu example against the unit), not more research.** |
