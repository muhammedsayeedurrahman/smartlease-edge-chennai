...
**Pre-existing code — clear this in the first hour.** Qualcomm's rules for its own edge-AI events forbid closed-source pre-existing code and require a prior proposal to be "significantly modified" during the event (confirmed 3-0). iQOO/Reskilll's specific wording was not verified. You carry roughly eleven days of pre-event work in a public repo. Ask an organiser directly, and make sure your Round-1 checkpoint (Sat 19:00-22:00) demonstrates substantial work done *during* the event. This is the only risk on this list that can void everything else.
